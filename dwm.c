...
enum { SchemeNorm, SchemeSel, SchemeInact }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? (m == selmon? SchemeSel:SchemeInact) : SchemeNorm]);
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeSel : SchemeInact]);
			// if (m != selmon) drw_setscheme(drw, scheme[SchemeInact]);
